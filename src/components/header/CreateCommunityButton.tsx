...
function CreateCommunityButton() {
    ...
    startTransition(async () => {
      try {
        ...
        if (!result) {
          ...
          const slug = result.subreddit.slug;
          if (slug) {
            setOpen(false);
            resetForm();
            router.push(`/community/${slug}`);
          } else {
            setErrorMessage("Slug missing from created community");
          }
        }
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger
        className="w-full p-2 pl-5 flex items-center rounded-md
        cursor-pointer bg-blue-600 hover:bg-blue-700 text-white
        dark:bg-[#0057ff] dark:hover:bg-blue-600
        whitespace-nowrap transition-all duration-200
        disabled:text-sm disabled:opacity-50 disabled:cursor-not-allowed"
        disabled={!user}
      >
          ...
          <form onSubmit={handleCreateCommunity} className="space-y-4 mt-2">
            ...
            <div className="space-y-2">
              ...
              <p className="text-xs text-gray-500">
                This will be used in the URL: aphive.com/community/
                {slug || "community-slug"}
              </p>
            </div>
              ...
              {imagePreview ? (
                ...
                <div className="flex items-center justify-center w-full">
                  <label
                    ...
                    h-24 border-2 border-gray-300 dark:border-gray-500 border-dashed rounded-lg
                    cursor-pointer bg-gray-50 dark:bg-[#202124] hover:bg-gray-100 
                    dark:hover:bg-[#2b2b2e]"
                  >
                    <div className="flex flex-col items-center justify-center">
                      <ImageIcon className="w-8 h-8 mb-2 mt-5 text-gray-400" />
                      <p className="text-xs text-gray-500">
            ...
            <Button
              ...
              font-medium py-2 transition-colors disabled:opacity-50
