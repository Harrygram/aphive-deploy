...
import { getUserGrowthData } from "./getUserGrowth";
...
export default function UserReportPage() {
  const [chartData, setChartData] = useState<MonthlyUserCount[]>([]);
  ...
  function handleExportCSV() {
    ...
    const rows = chartData.map((row) => [row.month, row.users]);
    const csvContent =
    ...
    link.download = "user_growth_report.csv";
    link.click();
  }
      ...
      <section className="bg-white border-b">
        <div className="mx-auto max-w-7xl px-4 py-4">
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-2xl font-bold">User Growth Report</h1>
              <p className="text-sm text-gray-600">
                View and analyze monthly new user signups
              </p>
            </div>
            <button
      ...
      <section className="bg-white">
        <div className="mx-auto max-w-7xl px-4 py-6">
          <div className="bg-white rounded-xl shadow p-6">
            <div className="mb-4">
              <h2 className="text-xl font-semibold">New Users Over Time</h2>
              <p className="text-gray-500">
                Showing total new users for each month
              </p>
            </div>
            ...
            <div className="mt-4 text-sm text-gray-600">
              ...
              <p className="text-muted-foreground">
                Data reflects user signups by month
              </p>
            </div>
          </div>
        </div>
